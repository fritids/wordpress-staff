...
?>
        <?php echo $before_widget; ?>
			...
			<form method="get" id="sidesearch" action="<?php bloginfo('home'); ?>/">
			<div>
			...
			<input type="text" value="<?php if($input==1){
				...
				} ?>" name="s" size="<?php echo $size; ?>" <?php if(strstr($other_text_mods, "id=") || strstr($other_text_mods, "style=")) { echo htmlspecialchars_decode($other_text_mods, ENT_QUOTES); } else { echo ''; } ?> /><?php echo $button_placement; ?><input type="<?php 
				if($button_type==1)
				{
				echo 'image" src="';
