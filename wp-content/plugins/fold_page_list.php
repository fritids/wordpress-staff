 ...
 */
 //wswwpx_fold_page_list
 
add_action("plugins_loaded", "fold_page_list_init");

function fold_page_list_init() {
register_sidebar_widget(__('Fold Page Side Menu'), 'fold_page_custom_menu');
register_widget_control(array('Fold Page Side Menu', 'widgets'),'fold_page_custom_menu_control', 250);

}

function fold_page_custom_menu_control() 
	{
	$options = get_option('fold_page_custom_menu');
	
	if ( $_POST['fold-page-submit'] ) 
		{
		$options['title'] = strip_tags(stripslashes($_POST['fold-page-title']));
		update_option('fold_page_custom_menu', $options);
		}
	
		echo '<p style="text-align:left;"><label for="fold-page-title">' . __('Title:') . ' <br /><input style="width: 200px;" id="fold-page-title" name="fold-page-title" type="text" value="'.$options['title'].'" /></label></p>';
	
		echo '<input type="hidden" id="fold-page-submit" name="fold-page-submit" value="1" />';
	//echo $before_widget;
	//echo $before_title;
	//echo $after_title;
	//echo $after_widget;
...
function fold_page_custom_menu($args) {
	extract($args);
	$options = get_option('fold_page_custom_menu');
